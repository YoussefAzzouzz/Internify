...
TECH_SKILLS = {
    ...
    "react", "node.js", "docker", "kubernetes", "c++", "c#",
    "typescript", "flask", "django", "spring", "angular",
    "vue", "tensorflow"
}
...
def extract_skills_with_ai(text):
    ...
    prompt = f"""Extract only the skills that are explicitly mentioned in the following resume text.
Do not infer or guess missing ones.
Return them strictly in this JSON structure:
{{"technical_skills": [], "non_technical_skills": []}}

Technical reference list: {', '.join(TECH_SKILLS)}
Non-technical reference list: {', '.join(NON_TECH_KEYWORDS)}
...
def extract_skills_regex(text):
    ...
    tech = [skill for skill in TECH_SKILLS if skill.lower() in tokens]
    non_tech = [skill for skill in NON_TECH_KEYWORDS if skill.lower() in tokens]
